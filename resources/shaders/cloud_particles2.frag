...
float sample_density(vec3 pos, mat3 up_matrix, vec3 ratio) {
    ...
    float d2 = max(0.0, texture(density_tex, fract((pn * 0.5 + 0.5) * vec3(0.1, 0.1, 0.1) + offset * vec3(-1.6, 0.5, -5.7))).g * 2.0 - 1.0);
    float d = max(0.0, texture(density_tex, fract((pn * 0.5 + 0.5) * vec3(0.6, 1.0, 2.5) + offset * vec3(8.0, 4.6, -1.9))).g * 2.0 - 1.0);
    float d1 = max(0.0, texture(density_tex, fract((pn * 0.5 + 0.5) * vec3(2.0, 2.0, 4.0) + offset * vec3(8.0, 4.6, -1.9))).g * 2.0 - 1.0);
    d += max(0.0, d1 * 0.5);

    //if(h2 % 4 == 0) v = vec3(1, 1, 8);
    //else if(h2 % 4 == 1 && size.x < 3500) v = vec3(1, 2, 1);

    d *= 1.0 - length(pn / ratio);
...
vec3 ellipsoid_raycast(vec3 center, vec3 radius, vec3 origin, vec3 dir, mat3 orientation) {
    ...
    vec3 new_origin = inv_ori * origin;
    vec3 new_center = inv_ori * center;

    vec3 rotated_dir = new_dir;

    new_dir = normalize(new_dir / radius);
    new_origin = new_origin / radius;
    new_center = new_center / radius;

    vec3 r = sphere_raycast(new_center, 1, new_origin, new_dir);
...
vec3 light_color_a = vec3(1.0, 0.8, 0.7);
...
void main() {
    ...
    rel_ro.xy = round(rel_ro.xy / size * num_pixels);

    rel_ro.xy += 0.5;

    rel_ro.xy = rel_ro.xy * size / num_pixels;

    rel_ro.z = 0;

    rel_ro = inv_view * rel_ro;

    //dir = normalize(origin2);

    vec3 dir = normalize(origin2 + rel_ro);

    vec3 ratio = vec3(1.0, 1.0, 0.125);

    //if(hash(uint(r.x * 6079)) % 2 == 0 && size.x < 3000) ratio = vec3(0.5, 0.5, 1.0);


    
    mat3 up_matrix = get_orientation(up);

    vec3 t = ellipsoid_raycast(vec3(0.0), size.x * ratio, rel_ro, dir, up_matrix);
    ...
    for(int i = 0; i < g; ++i) {
        ...
        vec3 u = ellipsoid_raycast(vec3(0.0), size.x * ratio, pos, sun_dir, up_matrix);
    ...
    float ndotl = dot(normalize(up), sun_dir);
    float wrap = 0.35;
