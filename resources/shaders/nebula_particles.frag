...
float sample_density(vec3 pos, mat3 up_matrix, vec3 ratio) {
    ...
    uint h1 = hash(uint(r.y * 1001)) ^ h;
    uint h2 = hash(uint(r.z * 90283) ^ h);
    uint h3 = hash(h1 ^ h);
...
float const_size = pow(2, 44);
float pixel_size = pow(2, 44) * 0.0625;
...
void main() {
    ...
    vec3 ratio = vec3(0.6, 1.0, 0.2);

    //if(size.x < 3000 && (hash(num) % 2 == 1)) ratio = vec3(1.0, 1.0, 1.0);
    ...
    for(int i = 0; i < g; ++i) {
        ...
        vec3 u = ellipsoid_raycast(vec3(0), size.x * ratio, pos, light_dir, up_matrix);
    ...
    vec3 dark_color = light_color * 0.35;
