...
float radius = 1.0;
float thickness = 0.66;

float sample_texture(vec3 v, float bar) {
    ...
    for(int i = 0; i < num_arms; ++i) {
        ...
        float rot_x = exp(nv.y / twist);

        float bulge_arm_falloff = 1.0;
        float fade = 0.1;
        //if(rot_x < 1.1) bulge_arm_falloff = 0.0;


        //nv = rot * nv;

        float thickness_2 = thickness * len;//(1.0 - abs((len - 0.5) / 0.5));
        thickness_2 = pow(thickness_2, 0.5);
        if(nv.y > 0.0) tex = max(tex, (thickness_2 - abs(nv.x)) / thickness_2 * bulge_arm_falloff);
    }
    ...
    float ss_scale = 0.1; // scale of distortion
    ...
    for(int i = 0; i < num_fluid_iterations; ++i) {
        ...
        float mix_v = 0.01; // amount of distortion
    ...
    */
    ...
    float tex_sample = sample_octaves((nv + vvv), 1, 1.0);
    tex_sample += (1.0 - length(v)) * 0.125;
    tex_sample = tex;
    ...
    if(intersection && thickness > 0.0) {
        ...
        if(i0 >= 0.0) {
            for(int i = 0; i < num_steps; ++i) {
                ...
                float density = sample_texture(pos3, max(b.x, b.y)); //  * data.period + data.noise_offset
                float falloff = max(0.0, (1.0 - pos3.z));
