...
vec3 color = vec3(0xCB, 0x3F, 0xAC) / float(0xFF);
...
float sample_texture(vec3 v) {
    ...
    float ss_scale = 4.0; // scale of distortion

    int num_fluid_iterations = 4;
...
vec3 galaxy_ray(vec3 ray_origin, vec3 ray_direction, float depth, galaxy_data data) {
    ...
    vec3 size = max(data.size, data.bulge_size);

    float i0;

    bool intersection = ray_plane(vec3(0.0), vec3(0.0, 0.0, 1.0), new_origin, new_direction, size.xy, i0);
    ...
    if(i0 > 0) {
        ...
        if(intersection) {
            ...
            if(i0 >= 0) {
                vec3 pos = new_origin + new_direction * i0;
                ...
                float t = 3; // amount of twisting
                t = log(len) * t;
                mat3 zr = z_rot(t);

                vec3 pos3 = zr * pos2;

                float density = sample_texture(pos3 * data.period + data.noise_offset);
                ...
                float bulge_factor = length(pos / data.bulge_size);
...
void main() {
    ...
    data.size = vec3(light_year * 30000.0);
    data.bulge_size = vec3(light_year * 6000.0);
    data.color = color;
    data.period = 1.25;
    ...
    data.size = vec3(light_year * 12000.0);
    data.bulge_size = vec3(light_year * 4000.0);
    ...
    data.size = vec3(0.0);
    data.bulge_size = vec3(light_year * 15000.0, light_year * 25000.0, light_year * 15000.0);
