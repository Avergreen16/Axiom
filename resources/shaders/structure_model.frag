...
layout(location = 0) in vec2 tex_world;
layout(location = 1) in vec3 normal;
layout(location = 2) in vec3 pos;
layout(location = 3) in vec4 tex_range;
...
void main() {
    ...
    vec2 tex_c = (mod(tex_world * world_to_pixel, tex_range.zw) + tex_range.xy) / texture_size;
    vec4 color = textureGrad(tex, tex_c, x, y);
