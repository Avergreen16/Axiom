#include <physics3d/system.hpp>
#include <physics3d/collider.hpp>
#include <physics3d/collision.hpp>
#include <physics3d/collision_constraint.hpp>
#include <physics3d/bounding_box.hpp>
...
void physics_system3d::broad_phase() {
    ...
    for(uint entity : collectors[0].entities) {
        ...
        collider3d& collider = axiom::ecs.get_component<collider3d>(entity);
        transform3d& transform = axiom::ecs.get_component<transform3d>(entity);
...
void physics_system3d::narrow_phase() {
    ...
    auto thread_GJK = [&](uint j) {
        for(ulong i : threads_collisions[j]) {
            ...
            collider3d& ai = axiom::ecs.get_component<collider3d>(a);
            collider3d& bi = axiom::ecs.get_component<collider3d>(b);

            uint ii = 0;
            uint jj = 0;

            if(!(ai.is_static && bi.is_static) && !(ai.collision_mask.contains(b) || bi.collision_mask.contains(a))) {
                collider3d& ac = axiom::ecs.get_component<collider3d>(a);
                transform3d& at = axiom::ecs.get_component<transform3d>(a);

                collider3d& bc = axiom::ecs.get_component<collider3d>(b);
                transform3d& bt = axiom::ecs.get_component<transform3d>(b);
    ...
    for(auto& [k, d] : collision_table) {
        for(manifold& manifold : d) {
            for(int j = 0; j < manifold.points.size(); ++j) {  
                ...
                cc.ca = &axiom::ecs.get_component<collider3d>(cc.a);
                cc.ta = &axiom::ecs.get_component<transform3d>(cc.a);
                
                if(cc.ca->collect) cc.ca->colliding_with.push_back(cc.b);

                if(cc.b != NULL_ENTITY) {
                    cc.cb = &axiom::ecs.get_component<collider3d>(cc.b);
                    cc.tb = &axiom::ecs.get_component<transform3d>(cc.b);
...
physics_system3d::physics_system3d() {
    signature s = axiom::update_signature<collider3d>();
    axiom::update_signature<transform3d>(s);
    ...
    while(abs(length) > precision && pos + length <= dist) {
        ...
        for(uint entity : collectors[0].entities) {
            if(!mask.contains(entity)) {
                collider3d& ec = axiom::ecs.get_component<collider3d>(entity);
                transform3d& et = axiom::ecs.get_component<transform3d>(entity);
...
void physics3d_init() {
    axiom::ecs.register_system(axiom::physics_system3d());
}
