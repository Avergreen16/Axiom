...
void collision_constraint::before() {
    ca = &axiom::ecs.get_component<collider3d>(a);
    ta = &axiom::ecs.get_component<transform3d>(a);
    if(b != NULL_ENTITY) {
        cb = &axiom::ecs.get_component<collider3d>(b);
        tb = &axiom::ecs.get_component<transform3d>(b);
    }
