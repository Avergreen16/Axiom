...
struct system_manager {
    std::unordered_map<std::size_t, std::unique_ptr<axiom::system>> systems;
    ...
    void register_system(type&& system) {
        ...
        systems.emplace(code, std::make_unique<type>(std::move(system)));
...
struct ecs_core {
    ...
    template<typename type> 
    type& get_system() {
        return *(type*)(system_manager_.systems[typeid(type).hash_code()].get());
    }
    
    template<typename type>
    void register_system(type&& system) {
        system_manager_.register_system<type>(std::move(system));
